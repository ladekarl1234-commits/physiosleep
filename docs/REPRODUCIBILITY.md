...
The protected [model-lineage crosswalk](../evidence/model-lineage-crosswalk-20260925.json) adds counts and hashes linking the frozen split, derived artifacts, 15 fold checkpoints, 357 out-of-fold predictions, the separate three-seed/transition development result and the D60 seed-17 model used on A/B. An independent verifier regenerated its private receipt and rejected seven mismatches; a methodologist reviewed the six-configuration screen and awarded only development-candidate credit. Public readers can inspect its aggregate identity graph but cannot rerun its private script or open excluded arrays. A complete, independently anchored byte-freeze map for all transition/audit artifacts remains absent, so full immutable ancestry and fresh-host training replay are not claimed.
...
A fresh local clone at the earlier release cutoff passed **92 synthetic tests**, exact aggregate arithmetic, and the tracked-file/link/source-hash scan in a separately created minimal Python 3.11 environment. The initial clone exposed Windows line-ending conversion; repository attributes now preserve exact source bytes. The corrected fresh clone passed all checks. See [the receipt](../evidence/clean-clone-verification.json). The current public suite has **99 tests**; the [hosted run for the reader/hardware evidence revision](https://github.com/ladekarl1234-commits/physiosleep/actions/runs/36100197203) passed at commit `bae06b3fdae41f70436673bf2e8516484cf01fa6`.
