...
[The saved-array verification receipt](../evidence/exploratory-audits-verification.json) records PASS for all 78 executions, full grids, metrics and 10,000-draw intervals. The verifier recomputed results independently rather than calling the project scoring function. At the exploratory-audit review cutoff, the simulated scores were **sleep specialist 73/100** and **data/ML 58/100** under unchanged rubrics. Later independent reviews raised the specialist grade to 74/100 for test-linked score sensitivity and the data/ML grade to 59/100 for complete source-byte provenance; see [the current grading rationale](JUDGING.md). These are project-evidence grades, not accuracy percentages.
