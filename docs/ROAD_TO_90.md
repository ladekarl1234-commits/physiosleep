...
criteria stay unchanged. Current independently reviewed grades are 61/100 for
...
The fixed [data/ML rubric](../evidence/judging/ml-exploratory.json) now scores [61/100](../evidence/judging/ml-candidate-lineage-regrade.json) after independently reviewed points for complete source-byte provenance, a 197-pair annotation/grid crosscheck and source-bound development candidate selection. The separate crosswalk still lacks a complete independently anchored byte map for all transition and audit artifacts, so the full hash-ancestry criterion remains 6/7. Mandatory comparators are 2/12, leaving 10 unavailable points under the stop; formal confirmation is 0/10, and both original holdouts are consumed, leaving the untouched-holdout/official-rules criterion at 0/3. Even **granting every other criterion full credit** gives an optimistic ceiling of **100 - 10 - 10 - 3 = 77/100**. If independent fresh-host full-training replay also remains absent (0/4), the ceiling is **73/100**. These are mathematical bounds, not predicted scores.
